...
class VirusTotalAnalyzer(Analyzer,AppBase):
    def __init__(self):
        Analyzer.__init__(self,'VirusTotal')
    ...
    def summary(self, raw):
        ...
        return {"taxonomies": taxonomies}
    def start_scan(self,file_path,filename):
        return self.read_scan_response(
                    self.vt.scan_file(file_path, from_disk=True, filename=filename),
                    self.wait_file_report,
                )
    @route('/scanfile/<workflow_id>')
    def scan_files(self,workflow_id):
        data = json.loads(self.redis_conn.get(workflow_id))
        if len(data)>0:
            for item in data:
                email_path = os.path.join(os.getcwd(),item["email_path"])
                filename = os.path.basename(email_path)
                result = self.start_scan(email_path,filename)
                item["scan_file_result"] = {"email":{},'attachments':[]}
                item["scan_file_result"]["email"] = result
                for attach in item["attachements_path"]:
                    att_filename = os.path.basename(attach)
                    att_result = self.start_scan(attach,att_filename)
                    att_result["attachement_path"] = attach
                    item["scan_file_result"]["attachments"].append(att_result)
                
            self.redis_conn.set(workflow_id,json.dumps(data))
        return {"data":data}
    def scan_hash(self,hash_value):
        print(f'result is ---------------------- {"result"}')
        result =  self.read_scan_response(
            self.vt.rescan_file(hash_value), self.wait_file_report
        )
