...
class Email(AppBase):
    ...
    def get_emails_imap(self):
        ...
        try:
            ...
            return {"success": False, "message": f"some data are required {e}"}

        def path_to_dict(path, value=None):
            def pack(parts):
                return {parts[0]: pack(parts[1:]) if len(parts) > 1 else value} if len(parts) > 1 else {parts[0]: value}
        ...
        try:
            ...
            for i in range(len(id_list) - 1, len(id_list) - amount - 1, -1):
                ...
                if upload_attachments:
                    ...
                    try:
                        output_dict["attachements_path"] = [
                            os.path.join(MEDIA_PATH, "attachements", email_id, x["filename"])
    ...
    def is_malicious_file(self, scans):
        ...
        if scans:
            ...
            return (
                ...
                or d_VBA32
                or d_TrendMicro
                or d_TrendMicro_HouseCall
                or d_Tencent
                or d_TACHYON
                or d_Symantec
                or d_Sophos
                or d_Sangfor
                or d_SUPERAntiSpyware
                or d_Rising
                or d_Panda
                or d_NANO_Antivirus
                or d_Microsoft
                or d_MicroWorld_eScan
                or d_McAfee_GW_Edition
                or d_McAfee
                or d_MaxSecure
                or d_Kingsoft
                or d_Malwarebytes
